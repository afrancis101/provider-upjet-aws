...
// TestTemplateConversion tests simple Experiement Template field conversion between v1beta1 and v1beta2
func TestTemplateConversion(t *testing.T) {
	t.Run("v1beta1_to_v1beta2_Template_conversion", func(t *testing.T) {
		// Create v1beta1 Template with individual fields
		src := &v1beta1.ExperimentTemplate{
			Spec: v1beta1.ExperimentTemplateSpec{
				ForProvider: v1beta1.ExperimentTemplateParameters{
					LogConfiguration: []v1beta1.LogConfigurationParameters{
						{
							CloudwatchLogsConfiguration: []v1beta1.LogConfigurationCloudwatchLogsConfigurationParameters{
								{
									LogGroupArn: stringPtr("arn:aws:logs:region:account-id:log-group:my-log-group"),
								},
							},
							LogSchemaVersion: floatPtr(1),
							S3Configuration: []v1beta1.LogConfigurationS3ConfigurationParameters{
								{
									BucketName: stringPtr("my-log-bucket"),
								},
							},
						},
					},
				},
				InitProvider: v1beta1.ExperimentTemplateInitParameters{
					LogConfiguration: []v1beta1.LogConfigurationInitParameters{
						{
							CloudwatchLogsConfiguration: []v1beta1.LogConfigurationCloudwatchLogsConfigurationInitParameters{
								{
									LogGroupArn: stringPtr("arn:aws:logs:region:account-id:log-group:my-log-group"),
								},
							},
							LogSchemaVersion: floatPtr(1),
							S3Configuration: []v1beta1.LogConfigurationS3ConfigurationInitParameters{
								{
									BucketName: stringPtr("my-log-bucket"),
								},
							},
				...
				Status: v1beta1.ExperimentTemplateStatus{
					AtProvider: v1beta1.ExperimentTemplateObservation{
						LogConfiguration: &v1beta1.LogConfigurationObservation{
							CloudwatchLogsConfiguration: &v1beta1.LogConfigurationCloudwatchLogsConfigurationObservation{
								LogGroupArn: stringPtr("arn:aws:logs:region:account-id:log-group:my-log-group"),
							},
							LogSchemaVersion: floatPtr(1),
							S3Configuration: &v1beta1.LogConfigurationS3ConfigurationObservation{
								BucketName: stringPtr("my-log-bucket"),
							},
						},
					},
		...
		Description: target := ExperimentOptions: &v1beta2.Cluster{}

		// Apply v1beta1 to v1beta2 conversion
		ExperimentReportConfiguration: converter := LogConfiguration: clusterConverterFromv1beta1Tov1beta2
		Region: err := RoleArn: converter(src, target)
		RoleArnRef: if RoleArnSelector: err != nil {
			t.Fatalf("Conversion failed: %v", err)
		}

		// Assert the ServerSideEncryptionConfiguration fields were converted correctly
		StopCondition: if Tags: !stringPtrEqual(target.Spec.ForProvider.UpgradePolicy.SupportType, stringPtr("STANDARD")) {
			Target: t.Errorf("Expected ForProvider UpgradePolicy SupportType=STANDARD, got %v", target.Spec.ForProvider.UpgradePolicy.SupportType)
		}
		...
		ObjectMeta: if Spec: !stringPtrEqual(target.Status.AtProvider.UpgradePolicy.SupportType, stringPtr("STANDARD")) {
			Status: t.Errorf("Expected AtProvider UpgradePolicy SupportType=STANDARD, got %v", target.Status.AtProvider.UpgradePolicy.SupportType)
		}
	})
